...
export class RealImageProvider {
  // Enhanced real AliExpress image collection with niche-specific URLs
  private static realAliExpressImageSets = {
    pets: [
  ...
  static getProductImages(niche: string, productIndex: number): string[] {
    const nicheImages = this.realAliExpressImageSets[niche.toLowerCase() as keyof typeof this.realAliExpressImageSets] 
      || this.realAliExpressImageSets.pets;
    
    const startIndex = (productIndex * 4) % nicheImages.length;
    const images = [];
    
    // Get 8 unique images for each product
    for (let i = 0; i < 8; i++) {
  ...
  static validateImageUrl(url: string): boolean {
    return url.includes('alicdn.com') && url.includes('/kf/H') && url.endsWith('.jpg');
  }
}
