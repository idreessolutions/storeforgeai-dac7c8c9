...
export class RealProductImageProvider {
  // High-quality, guaranteed working product images from multiple sources
  private static premiumImageSets = {
    pets: [
  ...
  static getRealProductImages(niche: string, productIndex: number): string[] {
    console.log(`🚨 REAL IMAGE PROVIDER: Getting images for ${niche} product ${productIndex}`);
    
    const nicheImages = this.premiumImageSets[niche.toLowerCase() as keyof typeof this.premiumImageSets] 
      || this.premiumImageSets.tech; // Fallback to tech images

    // Generate 8 unique images per product
    const startIndex = (productIndex * 3) % nicheImages.length;
    const images = [];
    
    for (let i = 0; i < 8; i++) {
      const imageIndex = (startIndex + i) % nicheImages.length;
      // Add unique cache-busting parameter for each product
      const imageUrl = `${nicheImages[imageIndex]}&cache=${productIndex}_${i}`;
      images.push(imageUrl);
    }
    
    console.log(`✅ REAL IMAGE PROVIDER: Generated ${images.length} images for ${niche} product ${productIndex}`);
    return images;
  }
